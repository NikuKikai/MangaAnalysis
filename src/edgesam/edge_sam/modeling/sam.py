...
class Sam(nn.Module):
    ...
    def __init__(
        ...
        if rpn_head == 'centernet':
            if CenterNetUpdateHead is None or FPN is None:
                raise ImportError("mmdet and mmengine are required to use the CenterNet RPN head.")
            self.fpn = FPN(
            ...
            if RPNHead is None or FPN is None:
                raise ImportError("mmdet and mmengine are required to use the RPN head.")
            self.fpn = FPN(
                in_channels=[96, 192, 384],
                out_channels=96,
                num_outs=5
            ...
            if efficientdet is None:
                raise ImportError("mmdet, mmengine, and EfficientDet project modules are required to use EfficientDet.")
